...
class Notif{
  ...
  Future <void> exportExcel({required dokumenCatatan, required String namafile,required BuildContext context}) async{
    ...
    cellL5.cellStyle = CellStyle(backgroundColorHex: "#B4C3E8",fontSize: 12,
        horizontalAlign: HorizontalAlign.Center);
    ...
    for (int i = 0; i < dokumenCatatan.length; i++) {
      ...
      String total = snapshot['Total Biaya'].toString();
      ...
      if (kebutuhanList.isNotEmpty) {
        for (var kebutuhan in kebutuhanList) {
          ...
          kebutuhanTextList.add(namaKebutuhan);
          statusTextList.add(status);
        }
        String kebutuhanText = kebutuhanTextList.join('\n');
        String statusText = statusTextList.join('\n');

        sheet.cell(CellIndex.indexByString("H$rowIndex")).value = TextCellValue(kebutuhanText);
        sheet.cell(CellIndex.indexByString("I$rowIndex")).value = TextCellValue(statusText);
      }
      ...
      if (biayaList != null && biayaList.isNotEmpty) {
        ...
        for (var biaya in biayaList) {
          ...
          biayaText += '$namaBiaya\n';
          hargaText +='$harga\n';
          biayaText = biayaText.replaceAll('\n', String.fromCharCode(10));
          hargaText = hargaText.replaceAll('\n', String.fromCharCode(10));
        }
    ...
    if (downloadsDirectory != null) {
      String filePath = '${downloadsDirectory.path}/${namafile}.xlsx';
